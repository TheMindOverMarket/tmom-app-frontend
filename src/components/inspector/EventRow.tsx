...
export const EventRow: FC<EventRowProps> = memo(({ event, rules = [] }) => {
  ...
  const bgColor = event.deviation ? 'rgba(234, 88, 12, 0.05)' : 'rgba(0, 255, 136, 0.03)';
  const iconColor = event.deviation ? '#EA580C' : 'var(--auth-accent)';
  const labelColor = event.deviation ? '#fb923c' : 'var(--auth-accent)';
  const tagBg = event.deviation ? 'rgba(234, 88, 12, 0.15)' : 'rgba(0, 255, 136, 0.1)';
